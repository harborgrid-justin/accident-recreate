import React, { useRef, useEffect, useMemo } from 'react';
import * as d3 from 'd3';
import { BarChartProps, TooltipData } from '../types';
import { DEFAULT_CHART_CONFIG, DEFAULT_INTERACTION } from '../index';
import ChartContainer from '../components/ChartContainer';
import ChartLegend from '../components/ChartLegend';
import ChartTooltip from '../components/ChartTooltip';
import { useLegendVisibility, useChartTooltip } from '../hooks/useChartData';

/**
 * Bar/Column chart with stacked and grouped options
 */
const BarChart: React.FC<BarChartProps> = ({
  data,
  config = {},
  interaction = {},
  horizontal = false,
  stacked = false,
  grouped = true,
  barWidth,
  onPointClick,
  className = '',
  style = {},
}) => {
  const svgRef = useRef<SVGSVGElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);

  const mergedConfig = { ...DEFAULT_CHART_CONFIG, ...config };
  const mergedInteraction = { ...DEFAULT_INTERACTION, ...interaction };

  const { visibility, toggleSeries, visibleSeries } = useLegendVisibility(data.series);
  const { tooltip, showTooltip, hideTooltip } = useChartTooltip<TooltipData>();

  const legendItems = useMemo(() => {
    return data.series.map((series, idx) => ({
      name: series.name,
      color: series.color || mergedConfig.colors.primary[idx % mergedConfig.colors.primary.length],
      visible: visibility[series.name] !== false,
      chartType: 'bar' as const,
    }));
  }, [data.series, visibility, mergedConfig.colors.primary]);

  useEffect(() => {
    if (!svgRef.current || !containerRef.current || visibleSeries.length === 0) return;

    const svg = d3.select(svgRef.current);
    const container = containerRef.current;
    const { width: containerWidth } = container.getBoundingClientRect();

    const width = mergedConfig.responsive ? containerWidth : mergedConfig.width;
    const height = mergedConfig.height;
    const margin = mergedConfig.margin;

    const boundedWidth = width - margin.left - margin.right;
    const boundedHeight = height - margin.top - margin.bottom;

    svg.selectAll('*').remove();
    svg.attr('width', width).attr('height', height);

    const g = svg
      .append('g')
      .attr('transform', `translate(${margin.left},${margin.top})`);

    // Get unique x values
    const xValues = Array.from(
      new Set(visibleSeries.flatMap((s) => s.data.map((d) => d.x)))
    ).sort((a, b) => a - b);

    // Prepare data
    let yMax: number;
    if (stacked) {
      const stackData = xValues.map((x) => {
        const point: any = { x };
        visibleSeries.forEach((series) => {
          const dataPoint = series.data.find((d) => d.x === x);
          point[series.name] = dataPoint ? dataPoint.y : 0;
        });
        return point;
      });

      const stack = d3.stack().keys(visibleSeries.map((s) => s.name));
      const seriesData = stack(stackData);
      yMax = d3.max(seriesData, (layer) => d3.max(layer, (d) => d[1])) || 0;
    } else {
      const allPoints = visibleSeries.flatMap((s) => s.data);
      yMax = d3.max(allPoints, (d) => d.y) || 0;
    }

    // Create scales
    const x0Scale = d3
      .scaleBand()
      .domain(xValues.map(String))
      .range(horizontal ? [0, boundedHeight] : [0, boundedWidth])
      .paddingInner(0.1)
      .paddingOuter(0.1);

    const x1Scale = d3
      .scaleBand()
      .domain(visibleSeries.map((s) => s.name))
      .range([0, x0Scale.bandwidth()])
      .padding(0.05);

    const yScale = d3
      .scaleLinear()
      .domain([0, data.yAxis.max !== undefined ? data.yAxis.max : yMax])
      .range(horizontal ? [0, boundedWidth] : [boundedHeight, 0])
      .nice();

    // Add grid
    if (data.yAxis.grid) {
      if (horizontal) {
        g.append('g')
          .attr('class', 'grid')
          .call(
            d3
              .axisBottom(yScale)
              .tickSize(boundedHeight)
              .tickFormat(() => '')
          )
          .call((g) => g.select('.domain').remove())
          .call((g) =>
            g
              .selectAll('.tick line')
              .attr('stroke', mergedConfig.colors.grid)
              .attr('stroke-opacity', 0.5)
          );
      } else {
        g.append('g')
          .attr('class', 'grid')
          .call(
            d3
              .axisLeft(yScale)
              .tickSize(-boundedWidth)
              .tickFormat(() => '')
          )
          .call((g) => g.select('.domain').remove())
          .call((g) =>
            g
              .selectAll('.tick line')
              .attr('stroke', mergedConfig.colors.grid)
              .attr('stroke-opacity', 0.5)
          );
      }
    }

    // Draw bars
    if (stacked) {
      const stackData = xValues.map((x) => {
        const point: any = { x };
        visibleSeries.forEach((series) => {
          const dataPoint = series.data.find((d) => d.x === x);
          point[series.name] = dataPoint ? dataPoint.y : 0;
        });
        return point;
      });

      const stack = d3.stack().keys(visibleSeries.map((s) => s.name));
      const seriesData = stack(stackData);

      seriesData.forEach((layer, idx) => {
        const series = visibleSeries[idx];
        const color = series.color || mergedConfig.colors.primary[idx % mergedConfig.colors.primary.length];

        const bars = g
          .selectAll(`.bar-${series.name.replace(/\s+/g, '-')}`)
          .data(layer)
          .join('rect')
          .attr('class', `bar-${series.name.replace(/\s+/g, '-')}`)
          .attr('fill', color)
          .style('cursor', 'pointer');

        if (horizontal) {
          bars
            .attr('x', (d) => yScale(d[0]))
            .attr('y', (d) => x0Scale(String(d.data.x))!)
            .attr('width', (d) => yScale(d[1]) - yScale(d[0]))
            .attr('height', x0Scale.bandwidth());
        } else {
          bars
            .attr('x', (d) => x0Scale(String(d.data.x))!)
            .attr('y', (d) => yScale(d[1]))
            .attr('width', x0Scale.bandwidth())
            .attr('height', (d) => yScale(d[0]) - yScale(d[1]));
        }

        bars
          .on('mouseover', function (event, d) {
            if (mergedInteraction.tooltip) {
              const rect = (event.target as SVGElement).getBoundingClientRect();
              showTooltip(rect.left + rect.width / 2, rect.top, {
                x: d.data.x,
                y: d[1] - d[0],
                label: String(d.data.x),
                value: d[1] - d[0],
                color,
                series: series.name,
              });
              d3.select(this).attr('opacity', 0.8);
            }
          })
          .on('mouseout', function () {
            hideTooltip();
            d3.select(this).attr('opacity', 1);
          });

        // Animate
        if (mergedConfig.animation.enabled) {
          if (horizontal) {
            bars
              .attr('width', 0)
              .transition()
              .duration(mergedConfig.animation.duration)
              .delay((_d, i) => i * 30)
              .attr('width', (d) => yScale(d[1]) - yScale(d[0]));
          } else {
            bars
              .attr('y', boundedHeight)
              .attr('height', 0)
              .transition()
              .duration(mergedConfig.animation.duration)
              .delay((_d, i) => i * 30)
              .attr('y', (d) => yScale(d[1]))
              .attr('height', (d) => yScale(d[0]) - yScale(d[1]));
          }
        }
      });
    } else {
      // Grouped bars
      visibleSeries.forEach((series, idx) => {
        const color = series.color || mergedConfig.colors.primary[idx % mergedConfig.colors.primary.length];

        const bars = g
          .selectAll(`.bar-${series.name.replace(/\s+/g, '-')}`)
          .data(series.data)
          .join('rect')
          .attr('class', `bar-${series.name.replace(/\s+/g, '-')}`)
          .attr('fill', color)
          .style('cursor', 'pointer');

        if (horizontal) {
          bars
            .attr('x', 0)
            .attr('y', (d) => x0Scale(String(d.x))! + (grouped ? x1Scale(series.name)! : 0))
            .attr('width', (d) => yScale(d.y))
            .attr('height', grouped ? x1Scale.bandwidth() : x0Scale.bandwidth());
        } else {
          bars
            .attr('x', (d) => x0Scale(String(d.x))! + (grouped ? x1Scale(series.name)! : 0))
            .attr('y', (d) => yScale(d.y))
            .attr('width', grouped ? x1Scale.bandwidth() : x0Scale.bandwidth())
            .attr('height', (d) => boundedHeight - yScale(d.y));
        }

        bars
          .on('mouseover', function (event, d) {
            if (mergedInteraction.tooltip) {
              const rect = (event.target as SVGElement).getBoundingClientRect();
              showTooltip(rect.left + rect.width / 2, rect.top, {
                x: d.x,
                y: d.y,
                label: d.label || String(d.x),
                value: d.y,
                color,
                series: series.name,
              });
              d3.select(this).attr('opacity', 0.8);
            }
          })
          .on('mouseout', function () {
            hideTooltip();
            d3.select(this).attr('opacity', 1);
          })
          .on('click', function (_event, d) {
            if (onPointClick) {
              onPointClick(d, series);
            }
          });

        // Animate
        if (mergedConfig.animation.enabled) {
          if (horizontal) {
            bars
              .attr('width', 0)
              .transition()
              .duration(mergedConfig.animation.duration)
              .delay((_d, i) => i * 30)
              .attr('width', (d) => yScale(d.y));
          } else {
            bars
              .attr('y', boundedHeight)
              .attr('height', 0)
              .transition()
              .duration(mergedConfig.animation.duration)
              .delay((_d, i) => i * 30)
              .attr('y', (d) => yScale(d.y))
              .attr('height', (d) => boundedHeight - yScale(d.y));
          }
        }
      });
    }

    // Add axes
    if (horizontal) {
      g.append('g')
        .attr('class', 'x-axis')
        .attr('transform', `translate(0,${boundedHeight})`)
        .call(d3.axisBottom(yScale))
        .call((g) =>
          g
            .selectAll('text')
            .attr('fill', mergedConfig.colors.text)
            .style('font-family', mergedConfig.font.family)
            .style('font-size', `${mergedConfig.font.size}px`)
        )
        .call((g) => g.select('.domain').attr('stroke', mergedConfig.colors.axis));

      g.append('g')
        .attr('class', 'y-axis')
        .call(d3.axisLeft(x0Scale))
        .call((g) =>
          g
            .selectAll('text')
            .attr('fill', mergedConfig.colors.text)
            .style('font-family', mergedConfig.font.family)
            .style('font-size', `${mergedConfig.font.size}px`)
        )
        .call((g) => g.select('.domain').attr('stroke', mergedConfig.colors.axis));
    } else {
      g.append('g')
        .attr('class', 'x-axis')
        .attr('transform', `translate(0,${boundedHeight})`)
        .call(d3.axisBottom(x0Scale))
        .call((g) =>
          g
            .selectAll('text')
            .attr('fill', mergedConfig.colors.text)
            .style('font-family', mergedConfig.font.family)
            .style('font-size', `${mergedConfig.font.size}px`)
        )
        .call((g) => g.select('.domain').attr('stroke', mergedConfig.colors.axis));

      g.append('g')
        .attr('class', 'y-axis')
        .call(d3.axisLeft(yScale))
        .call((g) =>
          g
            .selectAll('text')
            .attr('fill', mergedConfig.colors.text)
            .style('font-family', mergedConfig.font.family)
            .style('font-size', `${mergedConfig.font.size}px`)
        )
        .call((g) => g.select('.domain').attr('stroke', mergedConfig.colors.axis));
    }

    // Add axis labels
    if (data.xAxis.label) {
      g.append('text')
        .attr('x', boundedWidth / 2)
        .attr('y', boundedHeight + margin.bottom - 5)
        .attr('text-anchor', 'middle')
        .attr('fill', mergedConfig.colors.text)
        .style('font-family', mergedConfig.font.family)
        .style('font-size', `${mergedConfig.font.size + 2}px`)
        .text(data.xAxis.label);
    }

    if (data.yAxis.label) {
      g.append('text')
        .attr('transform', 'rotate(-90)')
        .attr('x', -boundedHeight / 2)
        .attr('y', -margin.left + 15)
        .attr('text-anchor', 'middle')
        .attr('fill', mergedConfig.colors.text)
        .style('font-family', mergedConfig.font.family)
        .style('font-size', `${mergedConfig.font.size + 2}px`)
        .text(data.yAxis.label);
    }
  }, [
    data,
    visibleSeries,
    mergedConfig,
    mergedInteraction,
    horizontal,
    stacked,
    grouped,
    barWidth,
    onPointClick,
    showTooltip,
    hideTooltip,
  ]);

  return (
    <div ref={containerRef} className={className} style={style}>
      <ChartContainer config={mergedConfig} title={data.title}>
        <svg ref={svgRef} />
      </ChartContainer>
      {mergedInteraction.legend && (
        <ChartLegend
          items={legendItems}
          config={mergedConfig}
          onToggle={toggleSeries}
        />
      )}
      {mergedInteraction.tooltip && <ChartTooltip data={tooltip} config={mergedConfig} />}
    </div>
  );
};

export default BarChart;
